...
class TableView extends Widget
{
     ...
     */
    public $templateCaption = '<th{captionOptions}>{label}</th>';
    public $templateContent = '<td{contentOptions}>{value}</td>';
    /**
     * @var array the HTML attributes for the container tag of this widget. The `tag` option specifies
     * what container tag should be used. It defaults to `table` if not set.
     * @see \yii\helpers\Html::renderTagAttributes() for details on how attributes are being rendered.
     */
    public $options = ['class' => 'table table-striped  table-responsive-lg  detail-view text-center'];
    /**
    ...
    public function run()
    {
        ...
        foreach ($this->attributes as $attribute) {
            $caption[] = $this->renderAttributeCaption($attribute, $i);
            $content[] = $this->renderAttributeContent($attribute, $i);
            $i++;
        }

        $options = $this->options;
        $tagTable = ArrayHelper::remove($options, 'tag', 'table');
        $tagCaption='<thead><tr>'.implode("\n", $caption).'</tr></thead>';
        $tagContent='<tbody><tr>'.implode("\n", $content).'</tr></tbody>';
        echo Html::tag($tagTable, $tagCaption.$tagContent, $options);
    }
     ...
     */
    protected function renderAttributeCaption($attribute, $index)
    {
        if (is_string($this->templateCaption)) {
            $captionOptions = Html::renderTagAttributes(ArrayHelper::getValue($attribute, 'captionOptions', []));

            if($attribute['value'] || $this->showEmptyContent)
            {
                return strtr($this->templateCaption, [
        ...
        return call_user_func($this->templateCaption, $attribute, $index, $this);
    }

    protected function renderAttributeContent($attribute, $index)
    {
        if (is_string($this->templateContent)) {
            $contentOptions = Html::renderTagAttributes(ArrayHelper::getValue($attribute, 'contentOptions', []));
            $contentOptions .= ' data-title="'.$attribute['label'].'" ';
            if($attribute['value'] || $this->showEmptyContent)
            {
                return strtr($this->templateContent, [
        ...
        return call_user_func($this->templateContent, $attribute, $index, $this);
    }
